/* 点礼物按钮验"点一件出一件"：真页面上按按钮文字点一件礼物，之后 5 秒每 0.2 秒记一次
   场上弹幕数（Ammo.count）和哥们（B）、闺蜜（G）在不在场。放到桌面 /home/op/shots/ 下跑（那里有 playwright-core）：
...
(async () => {
  ...
  for (const name of ['哥们', '闺蜜', '口红', '手柄', '香蕉']) {
    await p.evaluate((n) => [...document.querySelectorAll('[data-shop]')].find(x => x.textContent.startsWith(n)).click(), name);
    const seq = [];
    for (let i = 0; i < 25; i++) {
      seq.push(await p.evaluate(() => Ammo.count() + (Buddy.active() ? 'B' : '') + (Bestie.active() ? 'G' : '')));
