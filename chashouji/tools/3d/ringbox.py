...
# 转轴选 Y（绕相机轴转 = 正面锁定朝观众，在屏幕里打转），不是默认的 X 翻滚。
# 依据是"内容朝哪些方向分布"：开盖盒的内容（金环 + 钻）只朝上这一个方向，
# 绕 X 翻滚后半程盖子朝向镜头，整件读成一只空粉盒，戒指完全不可见（实测 8 个角度废 4 个）。
# 绕 Y 转则每一帧都能看见金环。
#
# axis='Y' 时：tilt -> 绕 Z = 偏航（转出 3/4 角，看出盒子的厚度和盖子的折角），
#             roll -> 绕 X = 俯仰（往前低头，看进盒子里的绒布槽）。
render_turntable('ringbox', active=body, tilt=0.42, roll=0.45, axis='Y')
