...
# 转轴用 lean=68（转轴从视线轴往屏幕横轴偏 68°），不是绕自身竖轴。
# 绕竖轴（axis='Z'）时杯子是个回转体，轮廓几乎不变、只有吸管在转 —— 实测剪影的主轴细长比
# 全程只波动 2%、可见面积只波动 8%，那就是一张贴纸在打旋，3D 白渲了。
# 斜轴 68° 之后：细长比 1.17~1.68（波动 44%），可见面积波动 20%，杯口那个椭圆会开合、
# 依次看到杯口、杯身、杯底。代价是转到正对杯底那两帧吸管被杯身挡住（36 帧里 6 帧），
# 45/60/68/75 四档实测：60 以上"吸管为零"的帧数就不再增加了（都是 2/12），
