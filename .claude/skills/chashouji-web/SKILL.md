...
实测位置：手机 y≈650，两人脸 y≈520~610，**墙面空白区 y=150~470**，HUD 整条占 y=4~133。
...
```js
const UI = { avR:33, avCX:46, avCY:37, barX:18, barW:442, barY:70, barH:42, pwY:118, pwH:15, sk:12 };
...
`TOP+26`，x 也改成 `frontAt(y)` 取同一行，免得指的位置和画的高度对不上。

四条规矩，踩过才知道：
- **数字不能放进条里**。压外端残血时和末端亮口叠在一起，压内端满血时被侵蚀带盖住 ——
  填充的末端迟早扫过整条，条里没有安全位置。移到条上方和队名并排。
- **拉力条要和血条留 6~8px**。贴着时两条深色描边并成一条粗黑带，拉力条读成"血条破了"。
