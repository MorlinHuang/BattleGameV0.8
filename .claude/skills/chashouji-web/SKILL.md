...
| `?swaystrip=N&swayms=M&swayp=P&swayfire=F` | 空闲拉锯胶片：**同一个 S.p** 只让时间往前走，每格标 pDraw 与手机 x。拉锯是纯时间函数，单张截图跟静止画面一模一样，两张不同时刻的截图又分不清是它在动还是页面没加载完。`swayfire=` 注入净差，验"有战况时拉锯有没有让位"：0 该满幅摆、230 起该完全停住 |
...
`BURN / LOSS / DPS` 是**同一根时间轴上的刻度，改一个必须三个一起按同样倍数改**。
它们分管两件**互相独立**的事，别混：

- **兑现速度** = `1/LOSS`。三个一起乘 k：稳态净差 ÷k 而 `DPS` ×k 正好抵消，
  稳态速度、均势时手机不动、弹幕总量全部守恒，只有"单次注入多久消化完"变。
  2026-09-21 整体 ×3.3（时间常数 83→25 秒），因为截图量出旧数值下送一件
  爆炸③**血条 20 秒纹丝不动、60 秒才跳 1 个百分点** —— 效果全在，只是摊太薄。
  副作用：碾压局快 17~24%（旧数值前八十秒火力还在爬坡）。
- **总推动力度** = `DPS / LOSS` 的比值。只调 `DPS` 能放大，但改玩法手感、缩短
  局长，`MATCH` 与绝杀阈值要跟着重配。**未拍板不要动。**
...
百分点，而单件礼物推得动多少？爆炸③ 1.53、最贵的空投④ 3.99，**一次摆动等于
2.5 个最贵礼物**，观众刷完最贵那件，12px 的推进埋在 30px 的晃动里，读出来就是
"我刷了什么都没发生"。所以它按 `FX.busy`（净差 / 100，趋近平滑）让位。
**让位指标要选直接决定目标量的那个**：用"火力总量"是错的 —— 双方对刷小礼物时
总量能稳在 1000，而那时净差是 0、进度一动不动，按总量判会把拉锯关死，僵局原样
回来，那恰恰是最需要它的场面。净差才是"画面在不在动"本身。
