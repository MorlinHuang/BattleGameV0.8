...
实测位置：手机 y≈650，两人脸 y≈520~610，**墙面空白区 y=150~470**，HUD 整条占 y=8~96。
...
顶上每侧是 **头像 · 队名+百分比 · 血条 · 拉力条** 一个整体（`UI` 表给几何，
右侧的 x 一律 `W - x - w` 推出来，改一处两边一起动）。中线是时钟 + 战况读数。

```js
const UI = { avR:38, avCX:60, avCY:58, barX:116, barW:298, barY:36, barH:30, pwY:74, pwH:11, sk:9 };
...
- **拉力条要和血条留 8px**。贴着时两条深色描边并成一条粗黑带，拉力条读成"血条破了"。
...
| `?hudflash=0..1` | 钉住拉力条的注入闪光。它只亮半秒，不钉住截图永远抓不到 |
