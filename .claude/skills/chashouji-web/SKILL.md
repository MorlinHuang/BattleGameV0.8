...
```js
const UI = { avR:30, avCX:44, avCY:35, barX:18, barW:442, barY:68, barH:58,
             clkCY:33, pwCY:172, sk:12 };
...
**横向顶满，竖向卡在指针上**（血条 298×30 → 442×42 → 442×58，两次放大）：
...
`TOP-2` → `TOP+36` → `TOP+102`（HUD 下沿 145 → 210）。改 HUD 高度必须同时改它，
两处对不上就会看见指针从血条中间划过去。指针的 x 用 `frontAt(y)` 取**同一行**，
免得指的位置和画的高度对不上。往下还剩多少：竖线到 282，人物帧顶 `FRAME_TOP` 是 308。
