---
name: chashouji-web
description: 《查手机》网页版的代码结构与改法——文件分工、几何常量、参数表 P、状态 S、派生 FX、礼物表 GIFT、URL 诊断参数、部署命令。用于"改网页""加一件礼物""调数值""对抗线位置不对""帧和线对不上""加个诊断参数""部署上去"这类请求。网页版是这个项目的单一真源，Godot 逐字对照搬。
---
...
`W - x - w` 推出来，改一处两边一起动）。中线**上**是时钟，中线**下**是拉力牌子
（2026-09-22 对调过，原来是反的）。
...
- 比倒计时显眼**两档**是有意的（85px vs 23px）—— 观众刷礼物改变的是这个数，
  不是那个钟；钟只在最后半分钟才重要，而那时它自己会变成"绝杀 18"。
- 牌子宽度**写死** 660（85px 等宽数字每位约 51px，两边各够五位数）。跟着数字
  宽窄伸缩的话，刷一件礼物牌子自己会抖，观众会以为画面卡了。上面时钟那块反过来
  要**跟着内容伸缩** —— 写死一个够宽的值会在"僵持"时空出一大片压在画面上。
- 注入闪光从菱形滑块改成**数字整个发光**。

```js
const UI = { avR:30, avCX:44, avCY:35, barX:18, barW:442, barY:68, barH:44,
             clkCY:33, pwCY:158, sk:12 };
// clkCY 时钟那一行（血条上面）· pwCY 拉力那一行（血条下面）—— 拉力不画条，见下
const HUD = { avA, avB, lfA, lfB, pfA, pfB, hfA, hfB, phA, phB, t, pin, hpin };
// 纯表现层，不参与胜负。hf/ph = 掉血闪光余量与上一帧血量；pin/hpin = 两个钉住参数
```

**时钟和拉力不等开局**：页面一打开就摆在那儿（时钟满时长、拉力 0:0）。等
`phase === 'play'` 才画的话，观众进直播间看到的是半块 HUD，第一件礼物砸下来
才突然冒出两行字，读成"卡了一下"。为此 `S.clock` 的初值是 `NUM.MATCH` 而不是 0
（否则画出来是 0:00），待机时不写"僵持"——那是对局里"谁都没掉血"的读数。

**掉血闪光**（2026-09-22 加）：血量**每跌过一个整点**闪一次 —— 整条压 50% 白、
末端亮口从 4px 张到 22px、外框渗一圈白光。
- 按"血量在变就亮"连续判会一直亮着，读出来是"这条是白的"，不是"正在掉"。
  按整点跨越，掉得越快闪得越密，节拍本身就是挨打的强度。
- 衰减 **7/秒**（约 0.14 秒灭）。4.5/秒试过：dps 3.2 时占空比七成，糊成常亮。
- 侵蚀带（`dps × 4.5` 宽）管的是"正在被啃"的持续态，闪光管"又掉了一点"的离散
  事件，两个都要 —— 血量是连续量，光有持续态读不出"刚刚掉了"。

**横向顶满，竖向卡在指针上**（血条 298×30 → 442×42 → 442×58 → 442×44）：
...
42px 宽横扫全宽，HUD 越过去就会被划一道。**这个顶不是定数，是跟着 HUD 一起推的**：
`TOP-2` → `TOP+36` → `TOP+102` → `TOP+88`（HUD 下沿 145 → 210 → 196）。改 HUD
高度必须同时改它，两处对不上就会看见指针从血条中间划过去。指针的 x 用
`frontAt(y)` 取**同一行**，免得指的位置和画的高度对不上。往下还剩多少：
竖线到 268，人物帧顶 `FRAME_TOP` 是 308。

**这块 UI 的形是一套，不是三套**（2026-09-22 统一）：血条是平行四边形（顶边整体
右移 `sk`），中线上的时钟和拉力板跨在正中，跟哪一侧对齐都会显得偏 —— 做成两端
对称内切的**梯形**（`plate` / `plateFill`），同一套切角语言又不偏向任何一边。
- 牌面：竖向渐变 + 顶边一道内亮线 + 投影。纯色平涂在照片底图上像贴了块塑料。
- 拉力板两端各一道队色亮边，**必须跟着斜边倾斜**（`skew` 而不是 `fillRect`）——
  竖着画的矩形会被梯形的斜边切掉大半，只剩底下一个小三角。
- 整片渗队色试过（两端各 120px、峰值 .26）：板本身是深的，队色叠上去只会变成
  一块墨绿和一块暗红，脏；待机时两个 0 也撑不住那么大两片颜色。细边亮度足、
  面积小，反而一眼分得清左右是谁的。
- 血条填充用**四段**渐变（1.30 / 1.12 / 1.0 / 0.52）+ 顶部高光收成渐变 +
  底边压一道暗：三段是"涂了渐变的色块"，四段才是"一根有圆度的管"。
- 两条血条中间那道 40px 缝**故意空着**。放过一个左右各半边队色的小菱形：满血时
  填充顶到缝边，菱形被夹成一条细缝，只剩"碎"。对峙感血条端部的斜切已经说清楚了。
...
| `?hphit=0..1` | 钉住掉血闪光（0.14 秒就灭）。0 / 0.35 / 0.7 / 1 截一排就是完整衰减形态 |
...
**钉住类参数（`?hudflash` `?hphit`）要在 URL 解析处当场把值写进状态**，不能
