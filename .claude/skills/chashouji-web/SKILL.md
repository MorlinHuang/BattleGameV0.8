...
顶上每侧是 **头像 · 队名+百分比 · 血条 · 拉力数值** 一个整体（`UI` 表给几何，
右侧的 x 一律 `W - x - w` 推出来，改一处两边一起动）。中线是时钟 + 战况读数。

**拉力写成"拉力 575"，不画条**（2026-09-21 改）。条要成立得有个量程，而拉力是
没有上限的存量（大哥一秒注入 600、对面只有 80 时差值能到四万），画条只能开方
压缩 —— 压完小额礼物推不动它、大额早早顶到头，两头都读不出来。写成数字两头都准：
刷一件就跳一截，跳多少和礼物的 `push` 值一一对应。而且这一层本来就不是"还剩多少"，
是"我砸进去了多少"，观众要对的是自己刷的那个数。注入闪光从菱形滑块改成**数字发光**。

```js
const UI = { avR:30, avCX:44, avCY:35, barX:18, barW:442, barY:66, barH:44, pwY:138, sk:12 };
// pwY 是**拉力那一行的文字基线**，不是条的 y —— 拉力早就不画条了，见下
